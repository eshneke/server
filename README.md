...
Написать приложение-сервер на C/C++ для GNU/Linux, 
который асинхронно обрабатывает клиентские подключения по протоколам TCP и UDP 
с помощью механизма мультиплексирования (на базе epoll) без использования сторонних библиотек.
...
Сервер должен поддерживать следующие команды:
 ...
 Makefile, systemd unit для запуска и пакет для дистрибутива.
